...
public class SmsNotificationListener {
	...
	@Autowired
	public SmsNotificationListener(ApplicationContext context, SMSService smsService,
	...
	@KafkaListener(topics = "${kafka.topics.notification.fsm.sms.name}")
	public void fsmSmsProcess(HashMap<String, Object> consumerRecord) {
		RequestContext.setId(UUID.randomUUID().toString());
		SMSRequest request = null;
		try {
			request = objectMapper.convertValue(consumerRecord, SMSRequest.class);
			if (request.getExpiryTime() != null && request.getCategory() == Category.OTP) {
				...
				if (expiryTime < currentTime) {
					...
					weblintoSMSServiceImpl.submitToExternalSmsService(request.toDomain());
				}
			} else {
				weblintoSMSServiceImpl.submitToExternalSmsService(request.toDomain());
			}
