...
public class OtpSMSRepository {
	...
	private static final String LOCALIZATION_KEY_FSM_LOGIN_SMS = "sms.rmc.login.otp.msg";
	...
	private String getMessageFormat(OtpRequest otpRequest) {
		...
		if (localisedMsgs.isEmpty()) {
			...
			localisedMsgs.put(LOCALIZATION_KEY_REGISTER_SMS,
					"Dear Citizen, Your OTP to complete your mSeva Registration is %s.");
			...
			localisedMsgs.put(LOCALIZATION_KEY_FSM_LOGIN_SMS, "OTP for login is %s Municipal Corporation Raipur");
		}
		String message = null;

		if (otpRequest.getModule() != null && otpRequest.getModule().equalsIgnoreCase("FSM")) {
			if (otpRequest.isLoginRequestType() || otpRequest.isRegistrationRequestType())
				message = localisedMsgs.get(LOCALIZATION_KEY_FSM_LOGIN_SMS);
		} else {
	...
	/**
	 * getRequiredTenantId() method return tenatid for loclisation as per the
	 * tenantIdStripSuffixCount. Example:- If provided tenantid is X.Y.Z and
	 * tenantIdStripSuffixCount = 1 then this function return X.Y as required tenant
	 * id for localisation. Depend on the value of tenantIdStripSuffixCount, the
	 * level of tenantid is removed from suffix of provided tenant id.
	 * 
	 * For tenantIdStripSuffixCount = 2 returns tenatId as X Similarly, for
	 * tenantIdStripSuffixCount = 3 or any other higher value will return top level
	 * tenantId (In this case it will return X as tenantId)
	 * 
	 * For tenantIdStripSuffixCount = 0 return tenantId as X.Y.Z here
	 * tenantIdStripSuffixCount as 0 means no cut from suffix.
	 * 
	 * 
	...
	private String getRequiredTenantId(String tenantId) {
		...
		if (tenantIdStripSuffixCount > 0 && tenantIdStripSuffixCount < tenantList.length) { // handeled case if
																							// tenantIdStripSuffixCount
