...
const BillDetails = ({ paymentRules, businessService }) => {
  ...
  const onSubmit = () => {
    ...
    if (window.location.href.includes("mcollect")) {
      history.push(`/fsm-ui/citizen/payment/collect/${businessService}/${consumerCode}?workflow=mcollect`, {
        paymentAmount,
        tenantId: billDetails.tenantId,
      });
    } else if (wrkflow === "WNS") {
      history.push(`/fsm-ui/citizen/payment/collect/${businessService}/${consumerCode}?workflow=WNS`, {
        paymentAmount,
        tenantId: billDetails.tenantId,
      });
    } else if (businessService === "PT") {
      history.push(`/fsm-ui/citizen/payment/billDetails/${businessService}/${consumerCode}/${paymentAmount}`, {
        paymentAmount,
        tenantId: billDetails.tenantId,
        name: bill.payerName,
      ...
      history.push(`/fsm-ui/citizen/payment/collect/${businessService}/${consumerCode}`, { paymentAmount, tenantId: billDetails.tenantId });
    }
  };
