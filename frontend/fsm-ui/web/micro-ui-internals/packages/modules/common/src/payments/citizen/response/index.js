  ...
  if (isError || !payments || !payments.Payments || payments.Payments.length === 0 || data.txnStatus === "FAILURE") {
    return (
      <Card>
        ...
        {business_service !== "PT" ? (
          <Link to={`/fsm-ui/citizen/fsm-home`}>
            <SubmitBar label={t("CORE_COMMON_GO_TO_HOME")} />
          </Link>
        ) : (
          <React.Fragment>
            <Link to={(applicationNo && `/fsm-ui/citizen/payment/my-bills/${business_service}/${applicationNo}`) || "/fsm-ui/citizen"}>
            ...
            <div className="link" style={isMobile ? { marginTop: "8px", width: "100%", textAlign: "center" } : { marginTop: "8px" }}>
              <Link to={`/fsm-ui/citizen/fsm-home`}>{t("CORE_COMMON_GO_TO_HOME")}</Link>
            </div>
          </React.Fragment>
        )}
      </Card>
