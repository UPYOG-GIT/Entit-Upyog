...
const CheckPage = ({ onSubmit, value }) => {
  ...
  let routeLink = `/fsm-ui/citizen/obps/sendbacktocitizen/${value.businessService=="BPA_OC" ? "ocbpa" : "bpa"}/${value?.tenantId}/${value?.applicationNo}`;
  if (value?.uiFlow?.flow === "OCBPA") routeLink = `/fsm-ui/citizen/obps/sendbacktocitizen/ocbpa/${value?.tenantId}/${value?.applicationNo}`;
    ...
    mutation.mutate(
      { BPA: { ...formdata?.BPA, workflow } },
      {
        ...
        onSuccess: (data, variables) => {
          ...
          history.replace(`/fsm-ui/citizen/obps/sendbacktocitizen/ocbpa/${value?.tenantId}/${value?.applicationNo}/acknowledgement`, { data: value?.applicationNo });
        },
      }
    );
  }
