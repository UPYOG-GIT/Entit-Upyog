    ...
    <div style={isOpenLinkFlow?{marginTop:"60px", width:isCitizenUrl?"100%":"70%", marginLeft:"auto",marginRight:"auto"}:{}}> */}
        ...
        <Card>
          ...
          {(mutation.isSuccess && !isOpenLinkFlow) && <Link to={{
            pathname: `/fsm-ui/citizen/payment/collect/${mutation.data.Licenses[0].businessService}/${mutation.data.Licenses[0].applicationNumber}/${mutation.data.Licenses[0].tenantId}?tenantId=${mutation.data.Licenses[0].tenantId}`,
            state: { tenantId: mutation.data.Licenses[0].tenantId },
          }}>
            <SubmitBar label={t("COMMON_MAKE_PAYMENT")} />
          </Link>}
          {!isOpenLinkFlow && <Link to={`/fsm-ui/citizen`}>
            <LinkButton label={t("CORE_COMMON_GO_TO_HOME")} />
          </Link>}
          {(mutation.isSuccess && isOpenLinkFlow) && <Link to={{
            pathname: `/fsm-ui/citizen`,
          }}>
