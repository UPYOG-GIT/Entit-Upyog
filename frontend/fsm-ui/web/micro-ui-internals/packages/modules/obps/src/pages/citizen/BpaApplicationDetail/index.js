...
const BpaApplicationDetail = () => {
  ...
  function onActionSelect(action) {
    ...
    if (action === "FORWARD") {
      history.replace(`/fsm-ui/citizen/obps/sendbacktocitizen/ocbpa/${data?.applicationData?.tenantId}/${data?.applicationData?.applicationNo}/check`, { data: data?.applicationData, edcrDetails: data?.edcrDetails });
    }
    if (action === "PAY") {
      window.location.assign(`${window.location.origin}/fsm-ui/citizen/payment/collect/${`${getBusinessServices(data?.businessService, data?.applicationStatus)}/${id}/${data?.tenantId}?tenantId=${data?.tenantId}`}`);
    }
    if (action === "SEND_TO_CITIZEN") {
      if (workflowDetails?.data?.processInstances?.length > 2) {
        window.location.replace(`/fsm-ui/citizen/obps/editApplication/${path}/${data?.applicationData?.tenantId}/${data?.applicationData?.applicationNo}`)
      } else {
    ...
    mutation.mutate(
      { BPA: { ...data?.applicationData, workflow } },
      {
        ...
        onSuccess: (data, variables) => {
          setIsEnableLoader(false);
          history.replace(`/fsm-ui/citizen/obps/response`, { data: data });
          setShowModal(false);
