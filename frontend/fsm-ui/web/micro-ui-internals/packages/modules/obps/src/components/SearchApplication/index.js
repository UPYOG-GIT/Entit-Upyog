...
const OBPSSearchApplication = ({ tenantId, t, onSubmit, data, error, searchData, isLoading, Count, setparamerror }) => {
  ...
  const { register, control, handleSubmit, setValue, getValues, reset, formState } = useForm({
    defaultValues: {
      applicationNo: "",
      mobileNumber: window.location.href.includes("/fsm-ui/citizen") ? currentUserPhoneNumber : "",
      fromDate: "",
      toDate: "",
      status: "",
      offset: 0,
  ...
  React.useEffect(() => {
    if (formState.isSubmitSuccessful) {
      ...
      reset({
        applicationNo: "",
       mobileNumber: window.location.href.includes("/fsm-ui/citizen") ? Digit.UserService.getUser()?.info?.mobileNumber : "",
        fromDate: "",
        toDate: "",
        status: "",
        offset: 0,
        ...
        {data?.[0]?.display ? (
          ...
          <DetailsCard
            {...{
              ...
              linkPrefix: window.location.href.includes("/citizen") ? `/fsm-ui/citizen/obps/` : `/fsm-ui/employee/obps/`,
