    ...
    return useMemo( () => ([
        {
          ...
          Cell: ({ row }) => {
            return (
              <div>
                <span className="link">
                  <Link to={window.location.href.includes("/citizen")  ? `/fsm-ui/citizen/obps/${getRedirectionLink(row.original["businessService"]) || "--"}/${row.original["applicationNo"] || row.original["applicationNumber"]}` : `/fsm-ui/employee/obps/${getRedirectionLink(row.original["businessService"]) || "--"}/${row.original["applicationNo"] || row.original["applicationNumber"]}`}>
