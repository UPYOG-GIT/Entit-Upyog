...
const OBPSResponse = (props) => {
  ...
  const getPaymentURL = (isCitizen) => {
    if (isCitizen == true) return `/fsm-ui/citizen/payment/collect/${getBusinessServices(applicationData?.businessService, applicationData?.status)}/${applicationData?.applicationNo}/${applicationData?.tenantId}?tenantId=${applicationData?.tenantId}`;
  }

  const getPaymentURLEmployee = () => {
    history.push(`/fsm-ui/employee/payment/collect/${getBusinessServices(applicationData?.businessService, applicationData?.status)}/${applicationData?.applicationNo}/${applicationData?.tenantId}?tenantId=${applicationData?.tenantId}`);
  }
              ...
              <div>
                {(applicationData?.status == "PENDING_APPL_FEE" || applicationData?.status == "PENDING_FEE" || applicationData?.status == "PENDING_SANC_FEE_PAYMENT") && billData?.length > 0 && isPayButtonEnable ?
                  <div>
                    ...
                    <Link to={`/fsm-ui/citizen`} >
                      <LinkButton label={t("CORE_COMMON_GO_TO_HOME")} />
                    </Link>
                  </div> :
                  <Link to={{ pathname: `/fsm-ui/citizen` }}>
