...
const EDCRAcknowledgement = (props) => {
  ...
  const routeToBPAScreen = async () => {
    history.push(
      `/fsm-ui/citizen/obps/bpa/${edcrData?.appliactionType?.toLowerCase()}/${edcrData?.applicationSubType?.toLowerCase()}/docs-required`,
      { edcrNumber: edcrData?.edcrNumber }
    );
    // window.location.assign(`${window.location.origin}/fsm-ui/citizen/obps/new-building-permit/docs-required`);
  }
      ...
      {edcrData.status == "Accepted" ?
        <Card style={{ padding: "0px" }}>
          ...
          <div style={{padding: "0px 10px"}}>
            <Link to={{pathname: `/fsm-ui/citizen/obps/${bpaLinks?.linkData?.flow?.toLowerCase()}/${edcrData?.appliactionType?.toLowerCase()}/${edcrData?.applicationSubType?.toLowerCase()}/docs-required`, state: bpaLinks}} replace>
            ...
            <div style={{marginTop: "12px", paddingBottom: "10px"}}>
              <Link to={`/fsm-ui/citizen/obps-home`} >
