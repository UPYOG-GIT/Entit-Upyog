...
const BPACitizenHomeScreen = ({ parentRoute }) => {
  ...
  if (userRoles && tradeType !== null && validityDate < currentDate.getTime()) {
      ...
      <div className="mainContent citizenAllServiceGrid">
        <Card>
          <CardText>{"Please Pay Renewal Fee for Renew your Registration, Application Number : " + consumerCode}</CardText>
          {
            <Link
              to={{
                pathname: `/fsm-ui/citizen/payment/collect/BPAREN/${consumerCode}/${userInfo?.info?.tenantId}?tenantId=${userInfo?.info?.tenantId}`,
              }}
            >
    ...
    const closeToast = () => {
      window.location.replace("/fsm-ui/citizen/all-services");
      setShowToast(null);
    };
    ...
    useEffect(() => {
      if (location.pathname === "/fsm-ui/citizen/obps/home") {
    ...
    const homeDetails = [
      {
        ...
        //     link: `/fsm-ui/citizen/obps/bpa/inbox`,
        //   },
        // ],
        kpis: [
          {
        ...
        links: [
          ...
          },
          {
            ...
            link: `/fsm-ui/citizen/obps/edcr/inbox`,
          },
        ],
