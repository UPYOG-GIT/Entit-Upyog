...
const EmployeeApp = ({ path, url, userType }) => {
  ...
  const breadcrumbObj = {
    ["/fsm-ui/employee/pt/inbox"]: "ES_TITLE_INBOX",
    ["/fsm-ui/employee/pt/new-application"]: "ES_TITLE_NEW_PROPERTY_APPLICATION",
    ["/fsm-ui/employee/pt/search"]: "PT_COMMON_SEARCH_PROPERTY_SUB_HEADER",
    ["/fsm-ui/employee/pt/application-search"]: "ES_COMMON_APPLICATION_SEARCH",
  };

  const getBreadCrumb = () => {
    if (breadcrumbObj[location.pathname]) return t(breadcrumbObj[location.pathname]);
    else if (location.pathname.includes("/fsm-ui/employee/pt/application-details/")) return t("PT_APPLICATION_TITLE");
    else if (location.pathname.includes("/fsm-ui/employee/pt/property-details/")) return t("PT_PROPERTY_INFORMATION");
    else if (location.pathname.includes("/fsm-ui/employee/pt/payment-details/")) return t("PT_PAYMENT_HISTORY");
    else if (location.pathname.includes("/fsm-ui/employee/pt/assessment-details/")) return t("PT_ASSESS_PROPERTY");
    else if (location.pathname.includes("fsm-ui/employee/pt/property-mutate-docs-required")) return t("PT_REQIURED_DOC_TRANSFER_OWNERSHIP");
    else if (location.pathname.includes("/fsm-ui/employee/pt/property-mutate/")) return t("ES_TITLE_MUTATE_PROPERTY");
    else if (location.pathname.includes("/fsm-ui/employee/pt/modify-application/")) return t("PT_UPDATE_PROPERTY");
  };
  ...
  const PTBreadCrumbs = ({ location }) => {
    ...
    const crumbs = [
      {
        path: "/fsm-ui/employee",
        content: t("ES_COMMON_HOME"),
        show: true,
      },
      {
        path: "/fsm-ui/employee/pt/inbox",
        content: t("ES_TITLE_INBOX"),
        show: location.pathname.includes("pt/inbox") ? true : false,
      },
      {
        path: "/fsm-ui/employee/pt/search",
      ...
      {
        path: `/fsm-ui/employee/pt/ptsearch/property-details/${sessionStorage.getItem("propertyIdinPropertyDetail")}`,
      ...
      {
        path: `/fsm-ui/employee/pt/applicationsearch/application-details/${sessionStorage.getItem("applicationNoinAppDetails")}`,
      ...
      <React.Fragment>
        <div className="ground-container">
          {/* <p className="breadcrumb" style={{ marginLeft: mobileView ? "2vw" : "revert" }}>
            <Link to="/fsm-ui/employee" style={{ cursor: "pointer", color: "#666" }}>
              {t("ES_COMMON_HOME")}
            </Link>{" "}
