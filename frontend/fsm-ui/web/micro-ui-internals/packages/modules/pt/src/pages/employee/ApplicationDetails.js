...
const ApplicationDetails = () => {
          ...
          nextActions: [
            {
              action: "VIEW_DETAILS",
              redirectionUrl: {
                pathname: `/fsm-ui/employee/pt/property-details/${propertyId}`,
              },
  ...
  if (
    ...
    workflowDetails?.data?.actionState?.nextActions.push({
      action: "UPDATE",
      redirectionUrl: {
        pathname: `/fsm-ui/employee/pt/modify-application/${propertyId}`,
    ...
    workflowDetails.data.actionState.nextActions = workflowDetails?.data?.actionState?.nextActions.map((act) => {
      if (act.action === "PAY") {
        return {
          ...
          redirectionUrl: { pathname: `/fsm-ui/employee/payment/collect/PT.MUTATION/${appDetailsToShow?.applicationData?.acknowldgementNumber}` },
        };
      }
      return act;
    });
