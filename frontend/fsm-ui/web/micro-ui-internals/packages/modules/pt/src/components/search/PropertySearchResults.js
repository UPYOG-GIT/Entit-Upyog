...
const SearchPTID = ({ tenantId, t, payload, showToast, setShowToast,ptSearchConfig }) => {
  ...
  const handleCollectTaxClick = (val) => {
    ...
    if(isAtleastOneMobileNumberInvalid) {
      ...
      revalidate();
      history.push(`/fsm-ui/employee/payment/collect/PT/${val?.["propertyId"]}`)
    }
  ...
  const skipNContinue = () => {
    history.push(`/fsm-ui/employee/payment/collect/PT/${selectedProperty?.['propertyId']}`)
  }
    ...
    () => [
      {
        ...
        Cell: ({ row }) => {
          return (
            <div>
              <span className="link">
                <Link to={`/fsm-ui/employee/pt/ptsearch/property-details/${row.original["propertyId"]}`}>{row.original["propertyId"]}</Link>
              </span>
            </div>
          );
        },
