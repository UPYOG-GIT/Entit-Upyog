  ...
  useEffect(() => {
    if (appDetailsToShow?.applicationDetails?.[0]?.values?.[1].title !== "PT_TOTAL_DUES") {
      appDetailsToShow?.applicationDetails?.unshift({
        ...
        belowComponent: () => (
          <LinkLabel
            onClick={() => history.push({ pathname: `/fsm-ui/employee/pt/ptsearch/payment-details/${applicationNumber}` })}
            ...
            ? [
              {
                ...
                customFunctionToExecute: (data) => {
                  ...
                  history.replace({ pathname: `/fsm-ui/employee/pt/ptsearch/assessment-details/${applicationNumber}`, state: { ...data } });
                },
              ...
              },
              {
                ...
                redirectionUrl: {
                  pathname: !fetchBillData?.Bill[0]?.totalAmount
                    ? `/fsm-ui/employee/pt/property-mutate-docs-required/${applicationNumber}`
                    : `/fsm-ui/employee/payment/collect/PT/${applicationNumber}`,
  ...
  if (appDetailsToShow?.applicationData?.status === "ACTIVE" && PT_CEMP) {
    ...
    if (!workflowDetails?.data?.actionState?.nextActions?.find((e) => e.action === "UPDATE")) {
      workflowDetails?.data?.actionState?.nextActions?.push({
        action: "UPDATE",
        redirectionUrl: {
          pathname: `/fsm-ui/employee/pt/modify-application/${applicationNumber}`,
