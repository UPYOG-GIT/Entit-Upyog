      ...
      mutate(data, {
        ...
        onSuccess: (data, variables) => {
          ...
          if (isOBPS?.bpa) {
            ...
            history.replace(`/fsm-ui/employee/obps/response`, { data: data });
          }
          if (isOBPS?.isStakeholder) {
            data.selectedAction = selectedAction;
            history.push(`/fsm-ui/employee/obps/stakeholder-response`, { data: data });
          }
          if (isOBPS?.isNoc) {
            history.push(`/fsm-ui/employee/noc/response`, { data: data });
          }
  ...
  function onActionSelect(action) {
    if (action) {
      if (action?.redirectionUrll) {
        window.location.assign(`${window.location.origin}/fsm-ui/employee/payment/collect/${action?.redirectionUrll?.pathname}`);
