                ...
                <div key={index}>
                  ...
                  {property?.institution && property?.institution?.type && <Row className="border-none" label={t("PT_INSTITUTION_TYPE")} text={`${t(`COMMON_MASTERS_OWNERSHIPCATEGORY_${property?.institution?.type}`) || t("CS_NA")}`} />}
                    <Row 
                      ...
                      actionButton={
                        <ActionButton style={{marginRight:"-10px"}} jumpTo={`/fsm-ui/citizen/pt/property/owner-history/${property.tenantId}/${property.propertyId}`} />
                      }
                    />
          ...
          <div>
            {property?.status === "ACTIVE" && !enableAudit && (
              <div style={{ marginTop: "1em", bottom: "0px", width: "100%", marginBottom: "1.2em" }}>
                <Link to={{ pathname: `/fsm-ui/citizen/pt/property/edit-application/action=UPDATE/${property.propertyId}` }}>
