...
const MyChallanResult = ({ template, header, actionButtonLabel }) => {
  ...
  const onSubmit = (data) => {
   history.push(`/fsm-ui/citizen/payment/my-bills/${data?.businesService}/${data?.ChannelNo}?workflow=mcollect`);
  };
  ...
  return (
    <div style={{ marginTop: "16px" }}>
      ...
      <div style={{ marginLeft: "16px", marginTop: "16px", marginBottom: "46px" }}>
        ...
        <p className="link">
          <Link to="/fsm-ui/citizen/mcollect/search">{t("UC_CLICK_HERE_TO_SEARCH_LINK")}</Link>
        </p>
      </div>
    </div>
  );
