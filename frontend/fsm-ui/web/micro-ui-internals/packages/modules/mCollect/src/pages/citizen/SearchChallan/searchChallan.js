...
const SearchChallan = ({ config: propsConfig, formData }) => {
  ...
  const onChallanSearch = async (data) => {
    //history.push(`/fsm-ui/citizen/mcollect/search-results`);
    ...
    } else if (!city.code)
    {
      ...
      history.push(
        `/fsm-ui/citizen/mcollect/search-results?mobileNumber=${mobileNumber}&challanNo=${challanNo}&Servicecategory=${
