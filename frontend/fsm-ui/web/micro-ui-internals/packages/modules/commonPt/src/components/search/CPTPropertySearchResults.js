  ...
  const columns = useMemo(
    () => [
              ...
              <span className="link">
                <span
                  onClick={() => {
                    ...
                    history.push(
                      `/fsm-ui/employee/commonpt/view-property?propertyId=${row.original["propertyId"]}&tenantId=${tenantId}&redirectToUrl=${redirectToUrl}`,
                      { ...state }
                    );
                  }}
                >
      ...
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => GetCell(row.original.owners.sort((a,b)=>a.additionalDetails.ownerSequence-b.additionalDetails.ownerSequence).map( o => o.name ). join(",") || "" ),
      },
