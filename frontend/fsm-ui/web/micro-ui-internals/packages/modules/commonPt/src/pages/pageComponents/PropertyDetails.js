...
const PropertyDetails = ({ t, config, onSelect, userType, formData }) => {
  ...
  function getChangePropertyPath() {
    ...
    return `/fsm-ui/citizen/ws/modify-connection/${formData?.tenantId}/search-property`
    else if(window.location.href.includes("/ws/edit-application/"))
    return `/fsm-ui/citizen/ws/edit-application/${formData?.tenantId}/search-property`
    else if(window.location.href.includes("/ws/"))
    return `/fsm-ui/citizen/ws/create-application/search-property`
    else if(window.location.href.includes("/renew-trade/"))
    return `/fsm-ui/citizen/tl/tradelicence/renew-trade/${formData?.applicationNumber}/${formData?.tenantId}/know-your-property`
    else if(window.location.href.includes("/edit-application/"))
    return `/fsm-ui/citizen/tl/tradelicence/edit-application/${formData?.applicationNumber}/${formData?.tenantId}/know-your-property`
    else
    return `/fsm-ui/citizen/tl/tradelicence/new-application/know-your-property`
  }
          ...
          <React.Fragment>
            ...
            <StatusTable>
              ...
              <div style={{ textAlign: "left" }}>
                <Link
                  to={`/fsm-ui/citizen/commonpt/view-property?propertyId=${propertyDetails?.Properties[0]?.propertyId}&tenantId=${propertyDetails?.Properties[0]?.tenantId}`}
                >
