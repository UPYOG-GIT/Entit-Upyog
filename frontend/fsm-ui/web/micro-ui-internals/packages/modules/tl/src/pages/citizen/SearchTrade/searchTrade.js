...
const SearchTrade = ({ config: propsConfig, onSelect }) => {
  ...
  const onTradeSearch = async (data) => {
    if (!data.mobileNumber && !data.LicenseNum) {
      ...
      history.push(
        `/fsm-ui/citizen/tl/tradelicence/renewal-list?mobileNumber=${data?.mobileNumber ? data?.mobileNumber : ``}&LicenseNumber=${data?.LicenseNum ? data?.LicenseNum : ``}&tenantId=${cityCode?cityCode:``}`
      );
    }
  };
