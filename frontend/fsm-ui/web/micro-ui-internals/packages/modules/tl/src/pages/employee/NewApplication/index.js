...
const NewApplication = () => {
  ...
  const onSubmit = (data) => {
          ...
          Digit.TLService.update({ Licenses: [licenses] }, tenantId)
            .then((response) => {
              if (response?.Licenses?.length > 0) {
                ...
                history.replace(`/fsm-ui/employee/tl/response`, { data: response?.Licenses });
    ...
    // history.replace("/fsm-ui/employee/tl/response", { Licenses: [formData], documents: applicationDocuments });
    // history.push("/fsm-ui/employee/pt/response", { Property: formData });
    // history.push("/fsm-ui/employee/pt/response", { Property: _formData });
  };
