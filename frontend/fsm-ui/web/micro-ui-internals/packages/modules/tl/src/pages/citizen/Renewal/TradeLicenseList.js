...
const TradeLicenseList = ({ application }) => {
  ...
  async function apicall(application) {
    ...
    if (isrenewalspresent && Licenses) {
      ...
      history.push(`/fsm-ui/citizen/tl/tradelicence/edit-application/action-edit/${application.applicationNumber}`);
    }
  }
  const onsubmit = () => {
    history.push(`/fsm-ui/citizen/tl/tradelicence/renew-trade/${application.licenseNumber}/${application.tenantId}`);
  }
  return (
    <Card>
