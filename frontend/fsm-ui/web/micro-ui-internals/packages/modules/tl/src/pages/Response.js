...
const Response = (props) => {
  ...
  const routeToPaymentScreen = async () => {
    window.location.assign(`${window.location.origin}/fsm-ui/employee/payment/collect/TL/${state?.data?.[0]?.applicationNumber}/${state?.data?.[0]?.tenantId}`);
  }
    ...
    <div>
        <Card>
          ...
          {state?.data?.[0]?.status !== "PENDINGPAYMENT" ?
            <Link to={`/fsm-ui/employee`} style={{ marginRight: "1rem" }}>
