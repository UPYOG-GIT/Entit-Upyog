...
const DEFAULT_REDIRECT_URL = "/fsm-ui/citizen/fsm-home";
...
const PropertySearchResults = ({
  ...
  if (oldPropertyIds || (searchQuery && searchQuery.oldPropertyIds))
    filters.oldPropertyIds = oldPropertyIds ? oldPropertyIds : searchQuery?.oldPropertyIds;
  ...
  const proceedToPay = (data) => {
    history.push(`/fsm-ui/citizen/payment/my-bills/PT/${data.property_id}`, { tenantId });
  };
  ...
  const onSubmit = (data) => {
    ...
    } else history.push(`/fsm-ui/citizen/payment/my-bills/PT/${data.property_id}`, { tenantId });
  };
  ...
  const sendOtpToUser = async (record) => {
    ...
    if (onSelect) {
      ...
      if (!err) {
        ...
        let redirectUrl = "/fsm-ui/citizen/commonPt/property/link-success/" + record.property_id;
        if (redirectToUrl) {
          redirectUrl = redirectToUrl + "?propertyId=" + record.property_id + "&tenantId=" + tenantId;
        }

        history.replace(`/fsm-ui/citizen/commonPt/property/citizen-otp`, {
        ...
        history.push(`/fsm-ui/citizen/`, { from: getFromLocation(location.state, searchParams), data: data });
      }
    }
  };
  ...
  return (
    <div className="static" style={{ marginTop: "16px" }}>
      <div className="static-wrapper">
        ...
        <ResponseComposer data={searchResults} template={template} actionButtonLabel={actionButtonLabel} onSubmit={sendOtpToUser} />
      </div>
      {modalData ? (
      ...
      {searchResults?.length !== 0 &&
        (searchResults?.length == 5 || searchResults?.length == 50) &&
        (locality || (searchQuery && searchQuery.locality)) && (
          <div>
            <p style={{ marginLeft: "16px", marginTop: "16px" }}>
              ...
              <span className="link">
                {
                  <Link
                    to={`/fsm-ui/citizen/pt/property/search-results?mobileNumber=${
