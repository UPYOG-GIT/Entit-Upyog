...
setCityCode(city.code);
  ...
  if (propertyData && !propertyDataLoading && !error && validation) {
    ...
    if ( !(searchData?.filters?.mobileNumber && Object.values(searchData?.filters)?.filter(ob => ob !== undefined)?.length == 1) && 
      ...
      if(redirectToUrl || window.location.href.includes("fsm-ui/citizen/commonpt/property/citizen-search")) {
        history.push(
          `/fsm-ui/citizen/commonPt/property/search-results?${Object.keys(qs)
            .map((key) => `${key}=${qs[key]}`)
  ...
  return (
    <div style={{ marginTop: "16px", marginBottom: "16px" ,backgroundColor:"white", maxWidth:"960px"}}>
      ...
      {window.location.href.includes("/obps/bpa/") ?<span className="link" style={isMobile ? {display:"flex", justifyContent:"center",paddingBottom:"16px"} : {display:"flex", justifyContent:"left",paddingBottom:"16px", marginLeft: "45px"}}>
        <Link to={"/fsm-ui/citizen/obps/bpa/building_plan_scrutiny/new_construction/location"}>{t("CORE_COMMON_SKIP_CONTINUE")}</Link>
      </span>:
           ...
           <Link to={window.location.href.includes("/ws/")?"/fsm-ui/citizen/ws/create-application/create-property" : window.location.href.includes("/tl/tradelicence/") ? "/fsm-ui/citizen/tl/tradelicence/new-application/create-property":window.location.href.includes("/fsm/")? "/fsm-ui/citizen/fsm/new-application/create-property":"/fsm-ui/citizen/commonpt/property/new-application"}>{t("CPT_REG_NEW_PROPERTY")}</Link>
         </span>}
      </div>
      {showToast && (
        <Toast
