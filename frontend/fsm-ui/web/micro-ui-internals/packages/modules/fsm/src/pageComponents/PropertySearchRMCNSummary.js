...
const PropertySearchRMCNSummary = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  const [searchMobileNumber, setSearchMobileNumber] = useState(urlPropertyId !== "null" ? urlPropertyId : "");
  ...
  const searchProperty = () => {
    ...
    if (window.location.pathname.includes("/tl/new-application")) {
      history.push(`/fsm-ui/employee/tl/new-application?propertyId=${propertyId}`);
      ...
      history.push(`/fsm-ui/employee/ws/new-application?propertyId=${propertyId}`);
  };
  ...
  const getOwnerNames = (propertyData) => {
    ...
    const getOwnersList = propertyData?.owners
        ...
        <div className="field" style={{ marginTop: "20px", display: "flex" }}>
          ...
          <LabelFieldPair>
            <CardLabel className="card-label-smaller" style={getInputStyles()}>
              {`${t("PROPERTY_ID_OR_MOBILE_NUMBER")}`}
            </CardLabel>

            {/* Separate the row with only inputs + button */}
            <div
              className="field"
              style={{
                marginTop: "10px",
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", width: "25%" }}>
                <label style={{ fontWeight: "bold", marginBottom: "4px" }}>{t("PROPERTY_ID")}</label>
                <TextInput
                  key="propertyId"
                  value={propertyId}
                  onChange={(e) => {
              ...
              {/* OR text */}
              <div style={{ fontWeight: "bold", marginTop: "20px" }}>OR</div>

              {/* Mobile Number input */}
              <div style={{ display: "flex", flexDirection: "column", width: "25%" }}>
                <label style={{ fontWeight: "bold", marginBottom: "4px" }}>{t("MOBILE_NUMBER")}</label>
                <TextInput
                  key="mobileNumber"
                  value={mobileNumber}
                  onChange={(e) => {
                    setMobileNumber(e.target.value);
                    onSelect("mobileNumber", { mobile: e.target.value });
                  }}
                />
              </div>
          ...
          {/* <span onClick={() => history.push(`/fsm-ui/employee/commonpt/search?redirectToUrl=${redirectBackUrl}&${serachParams}`, { ...state })}>
        <LinkButton label={t("CPT_SEARCH_PROPERTY")} style={{ color: "#a82227", display: "inline-block" }} />
      </span>
      &nbsp; | &nbsp;
      <span onClick={() => history.push(`/fsm-ui/employee/commonpt/new-application?redirectToUrl=${redirectBackUrl}&${serachParams}`, { ...state })}>
        <LinkButton label={t("CPT_CREATE_PROPERTY")} style={{ color: "#a82227", display: "inline-block" }} />
          ...
          {propertyDetails && propertyDetails?.Properties.length ? (
            <React.Fragment>
              ...
              <Link
                to={`/fsm-ui/employee/commonpt/view-property?propertyId=${propertyId}&tenantId=${tenantId}&from=${
...
export default PropertySearchRMCNSummary;
