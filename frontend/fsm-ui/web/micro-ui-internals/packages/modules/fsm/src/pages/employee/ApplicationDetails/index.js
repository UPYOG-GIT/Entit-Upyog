  ...
  useEffect(() => {
    switch (selectedAction) {
      ...
      case "ADDITIONAL_PAY_REQUEST":
        return history.push(`/fsm-ui/employee/payment/collect/FSM.TRIP_CHARGES/${applicationNumber}?workflow=FSM`);
      default:
        break;
    }
  }, [selectedAction]);
    ...
    if (checkpoint.status === "CREATED") {
      ...
      return (
        <div>
          ...
          <Link to={`/fsm-ui/employee/fsm/rate-view/${applicationNumber}`}>
      ...
      checkpoint.status === "START_WORK" ||
      checkpoint.status === "WORK_COMPLETED"
    ) {
      const caption = {
        date: checkpoint?.auditDetails?.created,
        name: checkpoint?.assigner,
