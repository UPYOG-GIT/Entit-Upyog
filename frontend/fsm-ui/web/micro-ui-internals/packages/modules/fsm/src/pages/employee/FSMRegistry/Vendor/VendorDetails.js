...
const VendorDetails = (props) => {
  ...
  useEffect(() => {
    switch (selectedAction) {
      ...
      case "EDIT":
        return history.push("/fsm-ui/employee/fsm/registry/modify-vendor/" + dsoId);
      case "HOME":
        return history.push("/fsm-ui/employee/fsm/registry?selectedTabs=VENDOR");
      default:
        break;
    }
  }, [selectedAction]);
  ...
  const handleVendorUpdate = () => {
    ...
    mutate(formData, {
      ...
      onSuccess: (data, variables) => {
        ...
        setTimeout(() => {
          closeToast();
          if (selectedAction === "DELETE") history.push(`/fsm-ui/employee/fsm/registry`);
        }, 5000);
      },
    });
    setShowModal(false);
  ...
  const onEdit = (details, type, id) => {
    if (type === "ES_FSM_REGISTRY_DETAILS_TYPE_DRIVER") {
      history.push("/fsm-ui/employee/fsm/registry/modify-driver/" + id);
    } else {
      let registrationNumber = details?.values?.find((ele) => ele.title === "ES_FSM_REGISTRY_VEHICLE_NUMBER")?.value;
      history.push("/fsm-ui/employee/fsm/registry/modify-vehicle/" + registrationNumber);
    }
  };
