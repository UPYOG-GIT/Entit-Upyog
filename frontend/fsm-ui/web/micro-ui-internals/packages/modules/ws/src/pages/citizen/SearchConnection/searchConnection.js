  ...
  const onConnectionSearch = async (data) => {
    if(searchType.code === "CONSUMER_NUMBER")
    {
      ...
      setShowToast({ key: true, label: "WS_HOME_SEARCH_CONN_RESULTS_DESC" });
        ...
        history.push(
          `/fsm-ui/citizen/ws/search-results?mobileNumber=${mobileNumber}&consumerNumber=${consumerNumber}&oldconsumerNumber=${oldconsumerNumber}&propertyId=${propertyId}&tenantId=${city.code}&locality=${locality.code}`
        );
      }
    }
    else
    {
      ...
      setShowToast({ key: true, label: "WS_HOME_SEARCH_CONN_RESULTS_DESC" });
        ...
        history.push(
          `/fsm-ui/citizen/ws/search-results?doorNumber=${doorNumber}&consumerName=${consumerName}&tenantId=${city.code}&locality=${locality.code}`
        );
        }
    } 
  };
