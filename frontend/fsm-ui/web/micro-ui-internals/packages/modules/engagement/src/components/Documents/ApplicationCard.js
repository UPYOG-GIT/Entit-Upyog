...
const ApplicationCard = ({
  ...
  const redirectToDetailsPage = (data) => {
    ...
    if (details) {
      history.push(`/fsm-ui/employee/engagement/documents/inbox/details/${details?.name}`, { details })
    }
  }
  ...
  if (data?.length === 0) {
    result = (
      <Card style={{ marginTop: 20, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        ...
        <Link className="link" to={`/fsm-ui/employee/engagement/documents/inbox/new-doc`}>{t('NEW_DOCUMENT_TEXT')}</Link>
      </Card>
    );
  }
