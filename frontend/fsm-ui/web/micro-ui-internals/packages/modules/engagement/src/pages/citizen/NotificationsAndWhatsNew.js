...
const NotificationsAndWhatsNew = ({ variant, parentRoute }) => {
  ...
  //     return <Redirect to={{ pathname: `/fsm-ui/citizen/login`, state: { from: location.pathname + location.search } }} />
  // }
  ...
  const VariantWiseRender = () => {
    switch (variant) {
      ...
      default:
        return <Redirect to={{ pathname: `/fsm-ui/citizen`, state: { from: location.pathname + location.search } }} />;
    }
  };
