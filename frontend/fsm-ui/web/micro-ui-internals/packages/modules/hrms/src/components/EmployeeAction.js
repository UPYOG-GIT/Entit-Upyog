...
const EmployeeAction = ({ t, action, tenantId, closeModal, submitAction, applicationData, billData }) => {
  ...
  function submit(data) {
    ...
    if (action !== "ACTIVATE_EMPLOYEE_HEAD") {
      ...
      history.replace("/fsm-ui/employee/hrms/response", { Employees, key: "UPDATE", action: "DEACTIVATION" });
    } else {
      if (file) {
        let documents = {
      ...
      history.replace("/fsm-ui/employee/hrms/response", { Employees, key: "UPDATE", action: "ACTIVATION" });
    }
  }
