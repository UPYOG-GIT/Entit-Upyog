...
const Login = ({ config: propsConfig, t, isDisabled }) => {
  ...
  useEffect(() => {
    ...
    let redirectPath = "/fsm-ui/employee";

    /* logic to redirect back to same screen where we left off  */
    if (window?.location?.href?.includes("from=")) {
      redirectPath = decodeURIComponent(window?.location?.href?.split("from=")?.[1]) || "/fsm-ui/employee";
    }

    /*  RAIN-6489 Logic to navigate to National DSS home incase user has only one role [NATADMIN]*/
    // if (user?.info?.roles && user?.info?.roles?.every((e) => e.code === "NATADMIN")) {
    //   redirectPath = "/fsm-ui/employee/payment/integration/dss/NURT_DASHBOARD";
    // }
    /*  RAIN-6489 Logic to navigate to National DSS home incase user has only one role [NATADMIN]*/
    // if (user?.info?.roles && user?.info?.roles?.every((e) => e.code === "STADMIN")) {
    //   redirectPath = "/fsm-ui/employee/payment/integration/dss/home";
    // }
    if (user?.info?.roles && user?.info?.roles?.every((e) => e.code === "NATADMIN")) {
      redirectPath = "/fsm-ui/employee";
    }
  ...
  const onForgotPassword = () => {
    history.push("/fsm-ui/employee/user/forgot-password");
  };
