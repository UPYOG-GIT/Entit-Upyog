...
const ForgotPassword = ({ config: propsConfig, t }) => {
  ...
  useEffect(() => {
    ...
    const redirectPath = location.state?.from || "/fsm-ui/employee";
  ...
  const onForgotPassword = async (data) => {
    ...
    try {
      ...
      history.push(`/fsm-ui/employee/user/change-password?mobile_number=${data.mobileNumber}&tenantId=${data.city.code}`);
    } catch (err) {
  ...
  const navigateToLogin = () => {
    history.replace("/fsm-ui/employee/login");
  };
