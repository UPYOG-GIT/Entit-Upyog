...
export const AppModules = ({ stateCode, userType, modules, appTenants }) => {
  ...
  if (!user || !user?.access_token || !user?.info) {
    return <Redirect to={{ pathname: "/fsm-ui/employee/user/login", state: { from: location.pathname + location.search } }} />;
  }
    ...
    <div className="ground-container">
      <Switch>
        ...
        <Route path={`${path}/login`}>
          {" "}
          <Redirect to={{ pathname: "/fsm-ui/employee/user/login", state: { from: location.pathname + location.search } }} />
        </Route>
