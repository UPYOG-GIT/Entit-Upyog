...
export const OBPSService = {
  ...
  BPADetailsPage: async (tenantId, filters) => {
    ...
    if (BPA?.businessService.includes("BPA_OC")) {
      applicationDetailsInfo["values"] = [...applicationDetailsInfo?.values, { title: "BPA_PERMIT_APP_NUMBER", to: `/fsm-ui/${envCitizenName}/obps/bpa/${bpaResponse?.BPA?.[0]?.applicationNo}`, value: bpaResponse?.BPA?.[0]?.approvalNo, isLink: true },];
